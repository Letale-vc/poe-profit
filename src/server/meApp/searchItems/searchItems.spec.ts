...
describe('SearchItems', () => {
  ...
  describe('makeARequestToAnyItem', () => {
    it('should throw an error if total is 0', async () => {
      ...
      mockPoeApi.poeFirsRequest.mockResolvedValue(firstResponse);

      await expect(
        searchItems.makeARequestToAnyItem(requestQuery),
      ).rejects.toThrowError('1');
      expect(mockPoeApi.poeFirsRequest).toHaveBeenCalledWith(requestQuery);
    });
    ...
    it('should make the second request with the correct arguments', async () => {
      ...
      mockPoeApi.poeFirsRequest.mockResolvedValue(firstResponse);
      mockPoeApi.poeSecondRequest.mockResolvedValue(secondResponse);
      ...
      expect(mockPoeApi.poeFirsRequest).toHaveBeenCalledWith(requestQuery);
      expect(mockPoeApi.poeSecondRequest).toHaveBeenCalledWith(
        ['item1', 'item2', 'item3'],
        'exampleId',
      );
    });
