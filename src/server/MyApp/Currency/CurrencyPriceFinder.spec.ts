...
describe('Currency', () => {
  ...
  test('should initialize currencyPrice with default values', () => {
    for (const val of CURRENCY_NAMES) {
      expect(currency.currencyPrice.get(val)).toBe(1);
    }
  });
  ...
  test('should update currency prices', async () => {
    ...
    jest.spyOn(poeApiMock, 'exchangeRequest').mockResolvedValue(searchResult);
    // Update currency prices
    await currency.update();
    // Assert that currency prices have been updated correctly
    expect(currency.currencyPrice.get('divine')).toBe(round(1200 / 5, 0));
