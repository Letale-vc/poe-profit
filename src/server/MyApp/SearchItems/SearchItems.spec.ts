...
const mockPoeApi = {
  httpRequest: {
    getWaitTime: jest.fn().mockResolvedValue(0),
    delay: jest.fn(),
  },
  leagueName: 'standard',
  firsRequest: jest.fn(),
  secondRequest: jest.fn(),
} as unknown as PoeTradeFetch;
...
describe('SearchItems', () => {
  ...
  describe('makeARequestToAnyItem', () => {
    it('should throw an error if total is 0', async () => {
      ...
      (mockPoeApi.firsRequest as jest.Mock).mockResolvedValue(firstResponse);

      await expect(
        searchItems.makeARequestToAnyItem(requestQuery),
      ).rejects.toThrowError('1');
      expect(void mockPoeApi.firsRequest).toHaveBeenCalledWith(requestQuery);
    });
    ...
    it('should make the second request with the correct arguments', async () => {
      ...
      (mockPoeApi.firsRequest as jest.Mock).mockResolvedValue(firstResponse);
      (mockPoeApi.secondRequest as jest.Mock).mockResolvedValue(secondResponse);
      ...
      expect(void mockPoeApi.firsRequest).toHaveBeenCalledWith(requestQuery);
      expect(void mockPoeApi.secondRequest).toHaveBeenCalledWith(
        ['item1', 'item2', 'item3'],
        'exampleId',
      );
    });
