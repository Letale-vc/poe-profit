...
import { type ObjectProfitDataType } from '~/server/MyApp/FileManagers';
...
export const createProfitDataColumns = (
...
): GridColDef<ObjectProfitDataType>[] => [
  {
    ...
    headerName: type === TYPE_DATA_NAMES.flipData ? 'Item buying' : 'Gem lvl 1',
    renderCell: (params) => (
      <Box
        display={'flex'}
        alignItems={'center'}
    ...
    minWidth: type === TYPE_DATA_NAMES.flipData ? 230 : 380,
  },

  {
    ...
    field: 'itemSelling',
    headerName:
      type === TYPE_DATA_NAMES.flipData ? 'Item selling' : 'Gem lvl 5',
    minWidth: type === TYPE_DATA_NAMES.flipData ? 230 : 380,
    renderCell: (params) => (
      <Box
        display={'flex'}
        ...
        width={'100%'}
      >
        <Link href={params.row.itemSelling.poeTradeLink} target="_blank">
  ...
  },
  {
    ...
    type: 'number',
    minWidth: 180,
    renderCell: (params) => params.row.itemBuying.price.fullStackSize.divine,
  },
  {
    disableColumnMenu: true,
    filterable: false,
